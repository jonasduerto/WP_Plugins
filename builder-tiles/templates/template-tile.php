...
if (TFCache::start_cache($mod_name, self::$post_id, array('ID' => $module_ID))):
    ...
    $container_class = implode(' ', apply_filters('themify_builder_module_classes', array(
        'module', 'module-' . $mod_name, $module_ID, 'size-' . $field_args['size'], $field_args['flip_effect'], $class, $field_args['css_class'], 'tile-type-front-' . $field_args['type_front'], $animation_effect
    ...
    ?>
    <!-- module tile -->
    <div <?php echo self::get_element_attributes($container_props); ?> data-auto-flip="<?php echo esc_attr($field_args['tile_autoflip']); ?>" data-in-effect="<?php echo $field_args['flip_effect']; ?>" data-out-effect="<?php echo $out_effect[$field_args['flip_effect']]; ?>">
            ...
            foreach ($sides as $side) : ?>
                ...
                <div class="tile-<?php echo $side; ?> tile-type-<?php echo $type; ?> ui <?php echo $field_args['color_' . $side]; ?>">
                    ...
                    <?php elseif ($type === 'button') :?>
                            <?php if ($field_args['button_link_'.$side] !== ''):?>
                                <?php $params_side = isset($field_args['button_link_params_'.$side])?$field_args['button_link_params_'.$side]:array();?>
                                <?php printf('<a href="%s" class="%s" %s>', in_array('lightbox', $params_side,true) ? esc_url($field_args['button_link_'.$side]) : esc_attr($field_args['button_link_'.$side]), in_array('lightbox', $params_side,true) ? ' themify_lightbox' : '', in_array('newtab', $params_side,true) ? ' target="_blank"' : '' ); ?>
                            <?php endif;?>
                            <?php if ('' !== $field_args['title_'.$side]) : ?>
                                <h4 class="tile-title"><?php echo wp_kses_post($field_args['title_'.$side]); ?></h4>
                            <?php endif; ?>
                    ...
                    <?php if (( $type === 'text' || $type === 'gallery' ) && '' !== $field_args['action_link_'.$side]): ?>
                        <?php $params_side = isset($field_args['action_param_'.$side])?$field_args['action_param_'.$side]:false;?>
                            <a href="<?php echo in_array('lightbox', $params_side,true) ? esc_url($field_args['action_link_'.$side]) : esc_attr($field_args['action_link_'.$side]); ?>" class="action-button
                                <?php if (in_array('lightbox', $params_side,true)):?> 
                                     themify_lightbox
                                <?php endif;?>"
                                <?php if (in_array('newtab', $params_side,true)): ?>
                                       target="_blank"
                                <?php endif;?>>
                                    <span></span> 
