...
class PTB_Public {
    ...
    private function _deregister_hooks() {
        remove_action('themify_post_start', array($this, 'filter_exceprt'), 1, 1);
        remove_filter('post_thumbnail_html', array($this, 'filter_exceprt'), 1, 1);
        remove_filter('get_the_excerpt', array($this, 'filter_exceprt'), 1, 1);
    }

    function register_template_directories() {
        if (empty($this->template_directories)) {
            $defaults = array(
                1 => trailingslashit(PTB::get_instance()->dir) . 'templates',
     ...
     * @return string|false
     */
    public function locate_template($names) {
        static $result = array();
        $names = (array) $names;
    ...
    public function enqueue_scripts() {
        ...
        wp_register_script($this->plugin_name, PTB_Utils::enque_min($plugin_url . 'js/ptb-public.js'), array($this->plugin_name . '-lightbox'), $this->version, false);
        ...
        wp_enqueue_style($this->plugin_name . '-lightbox', PTB_Utils::enque_min($plugin_url . 'css/lightbox.css'), array(), '0.9.9', 'all');
    ...
    public function ptb_filter_body_class($classes) {
        ...
        $type = self::$template ? 'archive' : ($templateObject->has_single() && is_singular($post_type) ? 'single' : false);
        if ($type !== false) {
    ...
    public function filter_content($content) {
        ...
        if (is_null($templateObject) || self::$render_content) {
			...
			return get_the_password_form(get_the_ID());
		} elseif (function_exists('wpml_get_language_information')) {
			$post_lang = wpml_get_language_information(get_the_ID());
			$lang = strtolower(PTB_Utils::get_current_language_code());
			if(strtolower($post_lang['language_code']) != $lang && ($lang != 'all' || !empty($lang))){
				 return !self::$render_content ? $content : '&nbsp;';
			}
		}
        ...
        if ($single || $archive) {
            ...
            if (isset($themplate_layout['layout']) && ($single || !in_array($id, self::$post_ids))) {
                self::$post_ids[] = $id;
                $render_template = $themplate->display_public_themplate($themplate_layout, $post_support, $cmb_options, $post_meta, $post_type, $single);
    ...
    public function filter_exceprt($content) {
        global $post;
        $post_type = $post->post_type;
        $this->_deregister_hooks();
        $templateObject = self::$options->get_post_type_template_by_type($post_type);
        if (is_null($templateObject) || self::$render_content) {
            ...
            return '&nbsp;';
        } elseif ( post_password_required() && is_singular($post_type) ) {
			return get_the_password_form(get_the_ID());
		} elseif (function_exists('wpml_get_language_information')) {
			$post_lang = wpml_get_language_information(get_the_ID());
			$lang = strtolower(PTB_Utils::get_current_language_code());
    ...
    public function filter_class($classes) {
        add_action('themify_post_start', array($this, 'filter_exceprt'), 1, 1);
        add_filter('post_thumbnail_html', array($this, 'filter_exceprt'), 1, 1);
        if (self::$template) {
            add_filter('get_the_excerpt', array($this, 'filter_exceprt'), 1, 1);
		}
    ...
    public function loop_start() {
        ...
        if (!self::$is_disabled) {
            ...
            if ($register === true) {
                ...
                add_filter('the_content', array($this, 'filter_content'), 100, 1);
            }
        } else {
    ...
    public function loop_end() {
        if (!self::$shortcode) {
            ...
            if (self::$loop_start === 0) {
                ...
                remove_filter('the_content', array($this, 'filter_content'), 100, 1);
    ...
    public function ptb_filter_cpt_category_archives(&$query) {
        ...
        if (!is_singular() && !is_feed($query) && ($query->is_category() || $query->is_post_type_archive() || $query->is_tag() || $query->is_tax()) && (!isset($query->query_vars['suppress_filters']) || $query->query_vars['suppress_filters'])) {
            ...
            if ($query->is_post_type_archive() && isset($query->query['post_type'])) {
				...
				ksort( $tax );
				$tax = current($tax);
				$taxonomy = ! empty( $tax['taxonomy'] ) ? get_taxonomy( $tax['taxonomy'] ) : '';
