...
class PTB {
    ...
    private function define_public_hooks() {
        ...
        $plugin_public = PTB_Public::get_instance();
        add_action('wp_head', array($plugin_public, 'ptb_filter_wp_head'));
        add_action('body_class', array($plugin_public, 'ptb_filter_body_class'));

        add_filter('pre_get_posts', array($plugin_public, 'ptb_filter_cpt_category_archives'), 99, 1);
        add_action('wp_enqueue_scripts', array($plugin_public, 'enqueue_scripts'), 99);
        add_action('wp_head', array($plugin_public, 'define_ajaxurl'));
        add_action('loop_start', array($plugin_public, 'loop_start'));
        add_filter('the_title', array($plugin_public, 'ptb_filter_post_type_title'), 100, 1);
        add_filter('post_thumbnail_html', array($plugin_public, 'ptb_post_thumbnail'), 100, 1);
