...
class PTB_CMB_Rating extends PTB_Extra_Base {
    ...
    public function action_public_themplate(array $args, array $data, array $meta_data, $lang = false, $is_single = false, $index = false) {
        ...
        <div itemprop="aggregateRating" itemscope itemtype="http://schema.org/AggregateRating"
            ...
            data-id="<?php echo $id ?>" 
            <?php if (isset($data['before_confirmation']) && !empty($data['before_confirmation_text'][$lang])): ?>
                data-before="<?php esc_attr_e($data['before_confirmation_text'][$lang]) ?>"
            <?php endif; ?>
            <?php if (isset($data['after_confirmation']) && !empty($data['after_confirmation_text'][$lang])): ?>
            ...
            <meta itemprop="ratingCount" content="<?php echo !$public && !empty($vote['count'])?$vote['count']:1?>"/>
        </div>
    ...
    public function voted() {
        if (isset($_POST['id']) && isset($_POST['key']) && isset($_POST['value'])) {
            ...
            $this->uid = get_current_user_id();
            if (
                    !empty($meta['readonly']) || ($value <= 0 || $value > $meta['stars_count'])
            ) {
                wp_die();
            }
